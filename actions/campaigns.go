...
// the path GET /campaigns/{campaignID}
func (v CampaignsResource) Show(c buffalo.Context) error {
	...
	// To find the Campaign the parameter campaignID is used.
...
// the path PUT /campaigns/{campaignID}
...
// to the path DELETE /campaigns/{campaignID}
...
func (v CampaignsResource) Destroy(c buffalo.Context) error {
	...
	if err := tx.Where("user_id = ?", userID).Where("campaignID = ?", campaign.ID).All(&quests); err != nil {
		if errors.Cause(err) != sql.ErrNoRows {
