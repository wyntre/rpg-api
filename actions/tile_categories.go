...
func (v TileCategoriesResource) Update(c buffalo.Context) error {
	...
	return c.Render(http.StatusAccepted, r.JSON(tile_category))
}
...
// List gets all TileTypes. This function is tile_typeped to the path
// GET /tile_types/{tile category id}
func (v TileCategoriesResource) Types(c buffalo.Context) error {
	tile_category_id, err := uuid.FromString(c.Param("id"))
	if err != nil {
		return c.Error(http.StatusInternalServerError, errors.New("bad tile category id"))
	}
	...
	// Retrieve all TileTypes from the DB
	if err := tx.Eager().Find(tile_category, tile_category_id); err != nil {
		return c.Error(http.StatusNotFound, errors.New("tile types not found"))
	}

	return c.Render(http.StatusOK, r.JSON(map[string]models.TileTypes{
		"tile_types": tile_category.TileTypes,
	}))
}
